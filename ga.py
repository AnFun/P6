...
class Individual_Grid(object):
  ...
  def mutate(self, genome):
    ...
    for y in range(height):
      for x in range(left, right):
        if block_count < 50 and y < height - 1 and (genome[y + 1][x - 1] == 'X' or genome[y - 1][x + 1] == 'X' or genome[y][x + 1]
                               or genome[y][x - 1]):
          if random.random() > 0.5 and (height - y) * random.random() < 1.5 and y > 8:
            genome[y][x] = 'X'
    return genome
...
def generate_successors(population):
  ...
  for x in range(200):
    randomthing = randgen(sorted_pop)
    for child in Individual.generate_children(sorted_pop[0], sorted_pop[randomthing]):
